        ...
        <c:forEach var="eventVO" items="${list }">
              ...
              <!--  new 날짜 -->
                ...
                <c:set var="sysdate"><fmt:formatDate value="${now}" pattern="yyyy-MM-dd" /></c:set>
                 <c:if test="${fn:substring(eventVO.event_rdate,0,10) == sysdate}">
                    <h3 class="event_new">NEW</h3>
                 </c:if>
							</a><!-- /.hover-effect -->  
							<span class="event_title" >  
                 <c:choose>
                   <c:when test="${fn:length(eventVO.event_title ) > 24}">
                     <c:out value="${fn:substring(eventVO.event_title,0,23)}"/>....
                   </c:when>
                   <c:otherwise>
                     <c:out value="${eventVO.event_title}"/>
                   </c:otherwise> 
                </c:choose>
              </span>   
