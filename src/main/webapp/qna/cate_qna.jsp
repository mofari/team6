    ...
    $.ajax({
      ...
      success: function(rdata) { // callback 함수 
        ...
        for(index=0; index < rdata.length; index++) {
          ...
          panel += "<TD style='text-align: center ;'>"+rdata[index].qnacategory_name+"</A></TD>";
          panel += "<TD style='text-align: center;'>"+rdata[index].qnacategory_ids+"</TD>";
          panel += "<TD style='text-align: center;'>"; 
          panel += "  <A href='../contents/create.do?qnacategory_no="+rdata[index].qnacategory_no+"'><IMG src='./images/create.png' title='등록'></A>";
          panel += "  <A href=\"javascript:update("+rdata[index].qnacategory_no+")\"><IMG src='./images/update.png' title='수정'></A>";  
          panel += "  <A href=\"javascript:deleteForm("+rdata[index].qnacategory_no+")\"><IMG src='./images/delete.png' title='삭제'></A>";
          panel += "</TD>";
          panel += "</TR>";
        }
        // alert(panel);
...
/*         var panel = '';
  ...
  function deleteForm(qnacategory_no) {
    $('#panel_create').hide();
    $('#panel_update').hide();
    $('#panel_delete').show();
    ...
    $.ajax({
      ...
      cache: false,
      dataType: "json", // 응답 데이터 형식, or json
      data: 'qnacategory_no=' +qnacategory_no,  // $('#frm').serialize(), 
      // Ajax 통신 성공, JSP 정상 처리
      success: function(rdata) { // callback 함수 
        var frm_delete = $('#frm_delete');
        $('#qnacategory_no', frm_delete).val(rdata.qnacategory_no);        
        $('#qnacategory_name').html(rdata.qnacategory_name); // 카테고리 이름
      },
      // Ajax 통신 에러, 응답 코드가 200이 아닌경우, dataType이 다른경우 
      error: function(request, status, error) { // callback 함수
        var panel = '';
...
//삭제 처리
  ...
  function update(qnacategory_no) {
    $('#panel_create').hide();
    $('#panel_update').show();
    ...
    $.ajax({
      ...
      success: function(rdata) { // callback 함수
        ...
        $('#qnacategory_no', frm_update).val(rdata.qnacategory_no);        
        $('#qnacategory_name', frm_update).val(rdata.qnacategory_name);
        $('#qnacategory_ids', frm_update).val(rdata.qnacategory_ids);        
      },
      // Ajax 통신 에러, 응답 코드가 200이 아닌경우, dataType이 다른경우 
      error: function(request, status, error) { // callback 함수
        var panel = '';
...
<DIV class='content' style="padding-top:10%;">
  ...
  <DIV id='panel_delete' style='padding: 10px 0px 10px 0px; background-color: #FFAAAA; width: 100%; text-align: center;'>
    <FORM name='frm_delete' id='frm_delete'>
      <input type='hidden' name='qnacategory_no' id='qnacategory_no' value=''>
  ...
  <DIV id='panel_create' style='padding: 10px 0px 10px 0px; background-color: #F5F5F5; width: 100%; text-align: center;'>
    <FORM name='frm_create' id='frm_create' method='POST' action='./create.do'>
      ...
      <input type='text' name='qnacategory_name' id='qnacategory_name' size='10' value='' required="required" style='width: 10%;'>

       
      <label for='ids'>접근 계정</label>
      <input type='text' name='qnacategory_ids' id='qnacategory_ids' value='admin' required="required" style='width: 10%;'>
  ...
  <DIV id='panel_update' style='padding: 10px 0px 10px 0px; background-color: #DDDDDD; width: 100%; text-align: center;'>  
    <FORM name='frm_update' id='frm_update' method='POST' action='./update.do'>
      ...
      <input type='text' name='qnacategory_name' id='qnacategory_name' size='15' value='' required="required" style='width: 20%;'>
