...
<html lang="en">
  ...
  <c:import url="/menu/top2.jsp" />
    <FORM name='frm' method='POST' action='./delete.do'>
       <input type='' name='review_no' value='${reviewVO.review_no}'>
      <input type="" name="category_no" value="${categoryVO.category_no }">
      <input type="" name="word" value="${param.word }">
      <input type="" name="nowPage" value="${param.nowPage }">
      ...
      <c:choose>
        <c:when test="${param.count == 0}">
           <div class="text">글 삭제에 실패했습니다.</div>
          <div class="text">다시한번 시도해주세요.</div>
            <br>
            <button type='button'  class="btn btn-default error-button" onclick='history.back()'>다시 시도</button>
						<a href="./list.do?category_no=${categoryVO.category_no}" class="btn btn-default error-button">목록으로</a>
        </c:when>
        <c:when test="${param.count == 1}">
          <h2 class="line-broken">글 삭제에 성공했습니다.</h2>
            <br>
            <a href="./list.do?category_no=${param.category_no}&word=${param.word }&nowPage=${param.nowPage }" class="btn btn-default error-button">목록으로</a>
        </c:when>
      </c:choose>    
					</div>
				</div>
...
<c:import url="../menu/js/review_delete_message_js.jsp" />
