...
<html lang="ko">
  ...
  function check_event(){
    if ($('#event_requestprize2').is(":checked"))
    {
      $('#event_requestprize').val("Y");
      alert("※이벤트 당첨");
    }else{
      $('#event_requestprize').val("N");
      alert("※이벤트 당첨 취소. 다시한번 확인하여 주세요");
    }
  }
</script>
 ...
 <FORM name='frm' method='POST' action='./update.do'
          ...
          <input type='hidden' name='event_requestno' id='event_requestno' value='${requestVO.event_requestno }'>
          <input type='hidden' name='member_no' id='member_no' value='1'>
          <input type='hidden' name='event_no' id='event_no'  value='${requestVO.event_no }'>
          
          <div style="padding: 30px;">

            <div class="info-block" id="basic">
              <div>
                <h3 class="title"  style="text-align: center; font-weight: 600;">이벤트 당첨 수정</h3>
                  <div class="checkbox checbox-switch switch-success" style="float: right;">
                   <input type="hidden" name="event_requestprize"  id="event_requestprize" value="${requestVO.event_requestprize }">
                         <label>
                         <c:if test="${requestVO.event_requestprize == 'Y' }">
                           <input type="checkbox" name="event_requestprize2"  id="event_requestprize2" value="${requestVO.event_requestprize }"  onclick="check_event()" checked=""/>
                         </c:if>
                         <c:if test="${requestVO.event_requestprize == 'N' }">
                           <input type="checkbox" name="event_requestprize2"  id="event_requestprize2" value="${requestVO.event_requestprize }"  onclick="check_event()">
                         </c:if>
                             <span></span>
                                 <div style="display: inline-block;font-size: 11px;color: #9f9f9f;">※ 이벤트 당첨 설정(ON/OFF)</div>
                           </label>
                       </div>
              </div>
              <div class="row">
                ...
                <div style="font-size: 13px; margin-bottom: 6px; color: #1fb7a6;"><b>이름</b></div>
                  <input id="event_requestman" class="form-control" type="text" placeholder="이름을 입력해주세요" name="event_requestman"  value="${requestVO.event_requestman }" required="required">
                </div>  
                
                <div  style="margin-bottom: 40px;">
                <div style="font-size: 13px; margin-bottom: 6px; color: #1fb7a6;"><b>전화번호</b></div>
                  <input id="event_requestphone" class="form-control"  placeholder="ex) 010-4244-5678"  type="tel" pattern="(010)-\d{3,4}-\d{4}"  name="event_requestphone"  value="${requestVO.event_requestphone }" required="required">
                </div>
                 
                <div>
                  <div style="font-size: 13px; margin-bottom: 6px; color: #1fb7a6;"><b>주소</b></div>
                  <input id="event_requestaddr" class="form-control" type="text" name="event_requestaddr"  value="${requestVO.event_requestaddr }" required="required">
                </div>
