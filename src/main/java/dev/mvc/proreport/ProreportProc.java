...
public class ProreportProc implements ProreportProcInter {
  ...
  public String paging(int search_count, int nowPage, String getparam) {
    int totalPage = (int)(Math.ceil((double)search_count/ContentsRpt.RECORD_PER_PAGE)); // 전체 페이지  
    int totalGrp = (int)(Math.ceil((double)totalPage/ContentsRpt.PAGE_PER_BLOCK));// 전체 그룹 
    int nowGrp = (int)(Math.ceil((double)nowPage/ContentsRpt.PAGE_PER_BLOCK));    // 현재 그룹 
    int startPage = ((nowGrp - 1) * ContentsRpt.PAGE_PER_BLOCK) + 1; // 특정 그룹의 페이지 목록 시작  
    int endPage = (nowGrp * ContentsRpt.PAGE_PER_BLOCK);             // 특정 그룹의 페이지 목록 종료   
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
    ...
    if (nowGrp >= 2){ 
      str.append("<span class='span_box_1'><A href='./list.do?nowPage="+_nowPage+getparam+"'>이전</A></span>"); 
    } 
    ...
    if (nowGrp < totalGrp){ 
      str.append("<span class='span_box_1'><A href='./list.do?nowPage="+_nowPage+getparam+"'>다음</A></span>"); 
    } 
    str.append("</DIV>"); 
